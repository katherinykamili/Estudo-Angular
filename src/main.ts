...
import { LearningEnvironment, Lesson, LessonSection, ModuleReview } from './types.js';
...
function codeBlock(code: string, language = 'texto'): string { return `<div class="code"><header><span>${esc(language)}</span><button data-copy="${encodeURIComponent(code)}">Copiar código</button></header><pre><code>${esc(code)}</code></pre></div>`; }

const labels: Record<LessonSection['kind'], string> = {
...
function sectionMarkup(section: LessonSection, index: number): string {
...
function environmentMarkup(environment: LearningEnvironment): string {
...
function reviewMarkup(review: ModuleReview, module: number): string {
...
function lessonPage(lesson: Lesson): string {
  ...
  if (!content) throw new Error(`Conteúdo explícito ausente para ${lesson.id}.`);
...
function bind(): void {
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-copy]').forEach((button) => button.addEventListener('click', async () => { await navigator.clipboard?.writeText(decodeURIComponent(button.dataset.copy ?? '')); const previous = button.textContent; button.textContent = '✓ Copiado'; setTimeout(() => { button.textContent = previous; }, 1400); }));
  ...
  search?.addEventListener('input', () => {
    ...
    results.hidden = !term;
    results.querySelectorAll<HTMLButtonElement>('[data-result]').forEach((button) => button.addEventListener('click', () => go(button.dataset.result ?? 'home')));
  });
}

const missing = lessons.filter((lesson) => !courseContent[lesson.id]);
if (missing.length) throw new Error(`Aulas sem conteúdo: ${missing.map((lesson) => lesson.id).join(', ')}`);
render();
